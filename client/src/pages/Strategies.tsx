...
const STRATEGY_TEMPLATES = [
  {
    ...
    description: "Buy/sell on fast/slow MA crossover points.",
    defaults: { timeframe: "1H", fast_ma: 10, slow_ma: 50 },
  },
  {
    name: "RSI Reversion",
    description: "Mean reversion using overbought/oversold RSI.",
    defaults: { timeframe: "1H", rsi_period: 14, overbought: 70, oversold: 30 },
  },
  {
    name: "MACD Trend",
    description: "Trend following using MACD histogram divergence.",
    defaults: { timeframe: "4H", fast: 12, slow: 26, signal: 9 },
  },
  {
    name: "Bollinger Bands",
    description: "Volatility breakout using Bollinger Bands.",
    defaults: { timeframe: "4H", period: 20, std_dev: 2.0 },
  },
  {
    name: "Volume Surge",
    description: "Trades on unusual volume spikes.",
    defaults: { timeframe: "15M", volume_multiplier: 2.0, lookback: 20 },
  },
];
    ...
    { label: "New Strategy", onClick: () => setShowCreateModal(true) },
    {
      ...
      onClick: () =>
        strategies?.forEach((s) =>
          updateStrategy.mutate({ id: s.id, isActive: false }),
        ),
    },
  ];
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Create and manage AI trading strategies
          </p>
        </div>
        <button
            ...
            {isLoading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-24 bg-secondary/40 rounded-lg animate-pulse"
                  />
                ))}
              </div>
            ) : !strategies?.length ? (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2.5 rounded-lg ${strat.isActive ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}
                        >
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded ${strat.isActive ? "bg-success/20 text-success" : "bg-muted text-muted-foreground"}`}
                        >
                  ...
                  {backtestRunning ? (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        {
                          label: "Win Rate",
                          val: backtestResults.winRate + "%",
                        },
                        {
                          label: "Total Trades",
                          val: backtestResults.totalTrades,
                        },
                        {
                          label: "Avg Return",
                          val: "+" + backtestResults.avgReturn + "%",
                        },
                        {
                          label: "Max Drawdown",
                          val: "-" + backtestResults.maxDrawdown + "%",
                        },
                        {
                          label: "Sharpe Ratio",
                          val: backtestResults.sharpeRatio,
                        },
                        {
                          label: "Profit Factor",
                          val: backtestResults.profitFactor,
                        },
                      ].map((item) => (
                        <div
                          key={item.label}
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewStratForm((prev) => ({
                      ...prev,
                      templateIdx: parseInt(e.target.value),
                    }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    STRATEGY_TEMPLATES[newStratForm.templateIdx].name
                  }
                  className="w-full px-4 py-2 bg-secondary/50 border border-border/50 rounded-lg text-foreground focus:outline-none focus:border-primary/50 text-sm"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={
                    STRATEGY_TEMPLATES[newStratForm.templateIdx].description
                  }
                  rows={2}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(
                    STRATEGY_TEMPLATES[newStratForm.templateIdx].defaults,
                  ).map(([k, v]) => (
                    <div
                      key={k}
              ...
              <div className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg">
                ...
                <Switch
                  ...
                  onCheckedChange={(val) =>
                    setNewStratForm((prev) => ({ ...prev, isActive: val }))
                  }
                />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border/60 rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              ...
              <strong>{selectedStrategy.name}</strong>. This action cannot be
              undone.
            </p>
