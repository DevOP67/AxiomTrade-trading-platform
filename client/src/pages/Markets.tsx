...
export default function Markets() {
  ...
  const filteredPairs = MARKET_PAIRS.filter((pair) => {
    ...
    const matchesFilter =
      filterBy === "all" || (filterBy === "active" ? pair.active : !pair.active);
            ...
            <div className="space-y-4">
              ...
              {[
                ...
                <div key={i} className="border-t border-border/50 pt-4">
                  ...
                  {row.val ? (
                    <p className="font-mono font-bold text-foreground">{row.val}</p>
                  ) : (
                ...
                <tr key={pair.symbol} className="hover:bg-secondary/30 transition-colors">
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${pair.active ? "bg-success" : "bg-muted-foreground"}`} />
