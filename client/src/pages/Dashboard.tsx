...
export default function Dashboard() {
  ...
  const { data: signals, isLoading: sigLoading, refetch: refetchSignals } = useSignals(undefined, 20);
  ...
  return (
    <div className="flex-1 overflow-auto bg-background p-4 md:p-6 lg:p-8 no-scrollbar">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard title="AI Confidence" value="High" subtitle="Signals aligned with models" highlight />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-2 xl:col-span-3 flex flex-col gap-6">
          <Widget title="BTC/USDT — AI Model Beta-1" className="h-[400px]" onRefresh={() => {}}>
                  ...
                  <thead className="bg-secondary/50 sticky top-0">
                    <tr>
                      {["Asset", "Size", "Entry", "PnL"].map((h) => (
                        <th key={h} className="py-3 px-4 text-xs text-muted-foreground font-medium uppercase tracking-wider text-right first:text-left">
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    {portLoading ? (
                      ...
                      return (
                        <tr key={pos.id} className="hover:bg-secondary/30 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right text-muted-foreground">{pos.amount}</td>
            ...
            <Widget title="AI Strategy Engine" className="h-[300px]">
              <div className="flex flex-col gap-3 overflow-y-auto h-full">
                {stratLoading ? (
                  ...
                  <div key={strat.id} className="flex items-center justify-between p-3 rounded-xl bg-secondary/40 border border-border/50 hover:border-primary/30 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${strat.isActive ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}>
                        <BrainCircuit className="w-4 h-4" />
                      </div>
                      <div>
                        <h4 className="font-semibold text-xs text-foreground">{strat.name}</h4>
                        <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-[150px]">{strat.description}</p>
                      </div>
                    </div>
                    <Switch
        ...
        <div className="lg:col-span-1">
          ...
          >
            <div className="overflow-auto h-full flex flex-col no-scrollbar">
              {sigLoading ? (
                <div className="p-4 text-center text-muted-foreground text-sm">Connecting to AI engine...</div>
              ) : !signals?.length ? (
                <div className="p-8 text-center text-muted-foreground flex flex-col items-center gap-2">
                  <Activity className="w-8 h-8 opacity-20" />
                  <p className="text-sm">No signals yet</p>
                  <p className="text-xs">Enable strategies to generate signals.</p>
                </div>
              ) : signals.map((sig, i) => {
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isBuy ? "bg-success/20 text-success" : "bg-danger/20 text-danger"}`}>
                          {sig.type}
                        </span>
                        <span className="font-bold text-sm">{sig.symbol}</span>
                      </div>
                      <span className="text-xs text-muted-foreground font-mono">
                        {sig.timestamp ? format(new Date(sig.timestamp), "HH:mm:ss") : "now"}
                      </span>
                    </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className="w-16 h-1.5 bg-secondary rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${sig.aiScore}%`, backgroundColor: getScoreColor(sig.aiScore) }}
                            />
                          </div>
...
function StatCard({ title, value, change, subtitle, isPositive, highlight }: {
  title: string; value: string; change?: string; subtitle?: string; isPositive?: boolean; highlight?: boolean;
}) {
  return (
    <div className={`bg-card p-4 rounded-xl border flex flex-col justify-between min-h-[100px] ${highlight ? "border-primary/50 shadow-[0_0_15px_rgba(59,130,246,0.1)] bg-gradient-to-br from-card to-primary/5" : "border-border/50 shadow-sm"}`}>
